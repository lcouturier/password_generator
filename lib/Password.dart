...
class Password {
  static String Function(int, CharType) _ofCore() {
    ...
    return (int size, CharType type) {
      var value = charTypes()
          ...
          .map((x) => d[x])
          .reduce((a, b) => "$a$b");
      return ArrayHelper.shuffle(Iterable.generate(10)
          .map((x) => ArrayHelper.shuffle(value).reduce((a, b) => "$a$b"))
          .reduce((a, b) => "$a$b")).take(size).reduce((a, b) => "$a$b");
    };
  }
  ...
  static const minus = '-';
  static const numbers = '01234567890123456789';
